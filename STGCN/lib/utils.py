...
def evaluate(net, test_loader, true_value, supports, device, epoch):
    ...
    with torch.no_grad():
        ...
        # 计算每个预测点的MAE、RMSE和MAPE
        mae_values = []
        rmse_values = []
        mape_values = []

        for i in range(12):
            print('current epoch: %s, predict point %s' % (epoch, i+1))

            mae = mean_absolute_error(true_value[:, :, i:i+1],
                                      prediction[:, :, i:i+1])
            rmse = mean_squared_error(true_value[:, :, i:i+1],
                                      prediction[:, :, i:i+1]) ** 0.5
            mape = masked_mape_np(true_value[:, :, i:i+1],
                                  prediction[:, :, i:i+1], 0)
        ...
        print('Average MAE: %.2f' % (avg_mae))
        print('Average RMSE: %.2f' % (avg_rmse))
        print('Average MAPE: %.2f' % (avg_mape))
