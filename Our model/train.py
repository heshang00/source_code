...
import argparse
import configparser
from model.OurModel import make_model
...
parser.add_argument(
    "--config",
    default="configurations/PEMS08.conf",
    type=str,
...
print(
    "total training epoch, fine tune epoch:",
...
filename_npz = os.path.join(
    dataset_name + "_r" + str(num_of_hours) + "_d" + str(num_of_days) +
    "_w" + str(num_of_weeks)
) + "_flag" + str(flag) + ".npz"
...
folder_dir = "MAE_%s_h%dd%dw%d_layer%d_head%d_dm%d_channel%d_dir%d_drop%.2f_%.2e" % (
    model_name, num_of_hours, num_of_days, num_of_weeks,
    num_layers, nb_head, d_model, encoder_input_size,
...
folder_dir = folder_dir + "flag" + str(flag)
...
train_loader, train_target_tensor, val_loader, val_target_tensor, \
    test_loader, test_target_tensor, _max, _min = load_graphdata_normY_channel1(
    graph_signal_matrix_filename, num_of_hours,
    num_of_days, num_of_weeks, DEVICE, batch_size, flag
)

net = make_model(
    DEVICE, num_layers, encoder_input_size, decoder_input_size,
    d_model, adj_mx, nb_head, num_of_weeks, num_of_days,
    num_of_hours, points_per_hour, num_for_predict,
    dropout=dropout,
...
def train_main():
    if (start_epoch == 0) and (not os.path.exists(params_path)):
        # Train from scratch, need to create new folder
        ...
        os.makedirs(params_path)
        print(
            "delete the old one and create params directory %s" % (params_path),
            flush=True
        )
    elif (start_epoch > 0) and (os.path.exists(params_path)):
        # Resume training, ensure the directory exists
        print("train from params directory %s" % (params_path), flush=True)
    else:
        raise SystemExit("Wrong type of model!")

    criterion = nn.L1Loss().to(DEVICE)  # Define loss function
    optimizer = optim.Adam(net.parameters(), lr=learning_rate)  # Define optimizer
    ...
    for epoch in range(start_epoch, epochs):
        ...
        print(
            "epoch: %s, train time every whole data:%.2fs" %
            (epoch, time() - train_start_time),
            flush=True
        )
        print(
            "epoch: %s, total time:%.2fs" %
...
def predict_main(epoch, data_loader, data_target_tensor, _max, _min, type):
    """
    Test the specified epoch on the test set
    ...
    predict_and_save_results(
        net, data_loader, data_target_tensor,
        epoch, _max, _min, params_path, type
    )
