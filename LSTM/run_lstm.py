...
parser.add_argument('--v_path', type=str, default='./data/PEMS08/pems08.csv')
parser.add_argument('--w_path', type=str, default='./data/PEMS08/pems08.csv')
...
def test():
    ...
    with torch.no_grad():
        ...
        for x, y in test_iter:
            ...
            ape += batch_size * masked_loss_np(y_pred, y_true, "mape")*100
            se += batch_size * masked_loss_np(y_pred, y_true, "rmse")

        mae1 = np.divide(ae, n)
        mape1 = np.divide(ape, n)
        rmse1 = np.sqrt(np.divide(se, n))

    print('Test loss {:.6f}'.format(test_loss))
    # print('MAE {:.6f} | MAPE {:.8f} | RMSE {:.6f} | mape2 {:.6f}'.format(mae, mape, rmse, mape2) )
    print('MAE {:.6f} | RMSE {:.8f} | MAPE {:.6f} '.format(mae1, rmse1, mape1) )
