...
class TrafficFlowData:
    ...
    def get_weighted_adjacency_matrix(self, sigma2=0.1, epsilon=0.5, scaling=True):  # 获取加权邻接矩阵
        df = pd.read_csv(self.w_path, header=0).to_numpy()
        return df

    def get_data(self):  # 读取数据
        df = pd.read_csv(self.v_path, header=0)
        len_df = len(df)
    ...
    def transform_data(self, data: np.ndarray):  # 转换数据
    ...
    def get_conv_kernel(self, approx: str):  # 获取卷积核
        if approx == "linear":
